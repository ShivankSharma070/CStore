...
// get_page returns a pointer to page numbered 'page_num' from pager. Allocates memory for a page if it does not exists
...
void pager_flush(Pager_t* pager, uint32_t page_num,uint32_t size) {
    ...
    ssize_t bytes_written= write(pager->file_discriptor, pager->pages[page_num], size);
    if (bytes_written == -1)  {
